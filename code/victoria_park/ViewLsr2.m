function landmarks=ViewLsr2

%function ViewLsr(FileName,figu,dttt)
% Jose. ACFR. 1999.

load 'ObservationDistance.txt';
load 'ObservationAngle.txt';
LL = size(ObservationDistance,1);
dd=ObservationDistance;
aa=ObservationAngle;
...
hhh3=plot(0,0,'k.','erasemode','xor') ;   % approx. landm. circles
axis([-50,50,0,75]);
hold off ;
...
for i=1:L,
	...
	ii = find(a>0) ;
	RR = RR/100 ; % cm to m
	xra=detectTreesI16(RR) ; % Tree Detection xra(1,:)-range, xra(2,:)-theta, xra(3,:)-landmark diameter
